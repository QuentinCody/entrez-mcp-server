...
export function registerBlastTools(server: McpServer) {
	...
	server.tool(
		...
		async ({
			...
			try {
				...
				const data = await parseResponse(response, "BLAST Submit");
		...
		async ({
			...
			try {
				...
				for (let i = 0; i < maxRetries; i++) {
					...
					const data = await parseResponse(response, "BLAST Get");
