import { z } from "zod";
import { BaseTool, ToolCapabilityDescriptor, ToolOperationDescriptor, ToolContext } from "./base.js";
...
export class CapabilitiesTool extends BaseTool {
	...
	override register(): void {
		this.context.server.tool(
			...
			"List the available tools, their operations, and guidance for usage.",
			{
				...
				format: z.enum(["summary", "detailed", "json"]).default("summary").describe("Response style"),
				include_metadata: z.boolean().optional().describe("Include extended metadata when available"),
			},
			async params => {
	...
	override getCapabilities(): ToolCapabilityDescriptor {
		return {
			...
			summary: "Inspect available tools, operations, token profiles, and auth requirements.",
	...
	private respondSummary(descriptors: ToolCapabilityDescriptor[]) {
		const lines = descriptors.map(cap => {
	...
	private respondDetailed(descriptors: ToolCapabilityDescriptor[], includeMetadata?: boolean) {
		const sections = descriptors.map(cap => {
			...
			if (cap.requiresApiKey) metaLines.push("- Requires API key for optimal throughput");
			if (cap.stageable) metaLines.push("- Supports durable staging for large responses");
			if (cap.tokenProfile) {
		...
		return {
			content: [{
				type: "text" as const,
				text: `**Tool Capability Guide**\n\n${sections}`,
			}],
		};
	}

	private respondWithJson(descriptors: ToolCapabilityDescriptor[], includeMetadata?: boolean) {
		const sanitized = descriptors.map(cap => {
			if (includeMetadata) return cap;
			const { metadata, ...rest } = cap;
			return rest;
		});
	...
	private formatOperation(operation: ToolOperationDescriptor): string {
		const requiredParams = operation.required.length > 0
			? `Required: ${operation.required.map(param => this.formatParam(param)).join(", ")}`
			: "Required: none";
		const optionalParams = operation.optional.length > 0
			? `Optional: ${operation.optional.map(param => this.formatParam(param)).join(", ")}`
			: "Optional: none";
		const remarks = operation.remarks?.map(note => `- ${note}`).join("\n");
		return [`**${operation.name}** — ${operation.summary}`, requiredParams, optionalParams, remarks]
			.filter(Boolean)
			.join("\n");
	}

	private formatParam(param: { name: string; type: string; description: string; defaultValue?: unknown; values?: string[]; }): string {
		const defaultSuffix = param.defaultValue !== undefined ? ` (default: ${param.defaultValue})` : "";
		const valuesSuffix = param.values && param.values.length > 0 ? ` [${param.values.join(", ")}]` : "";
