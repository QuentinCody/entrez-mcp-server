		...
		this.context.server.tool(
			...
			"Fetches and PARSES data from Entrez databases, then stages it into a relational SQLite database with proper entity extraction.",
			{
				db: z
					.string()
					.default("pubmed")
					.describe("Database name (e.g., 'pubmed', 'protein', 'nuccore')"),
				...
				rettype: z
					...
					.default("xml")
					.describe(
						"Data format (e.g., 'xml', 'fasta', 'gb'). Determines which parser to use.",
					),
			},
			async ({ db, id, rettype }) => {
				try {
					...
					const shouldBypass = this.context.shouldBypassStaging(
						processedData,
					...
					if (
						stagingDetails.total_rows <= 15 &&
						stagingDetails.table_count <= 2
					) {
						return {
							content: [
								{
									...
									text: JSON.stringify(
										{
											...
											message:
												"Simple dataset staged but returned directly for efficiency",
											entity_count: entityCount,
											data: processedData,
...
## 🚀 Quick Start Queries:
${
	details.schema_guidance?.recommended_queries
		?.slice(0, 3)
		.map((q: any, i: number) => `${i + 1}. \`${q.sql}\` - ${q.description}`)
		.join("\n") ||
	...
	register(): void {
		this.context.server.tool(
			...
			"Execute SQL queries against previously staged Entrez datasets. Query the relational database created by efetch_and_stage with full SQL support including JOINs, aggregations, and complex filtering across parsed biomedical data.",
			{
				data_access_id: z
					.string()
					.describe("The data_access_id from a tool call that staged data."),
				...
				try {
					...
					return {
						content: [
							{
								type: "text",
								text: "Error: Invalid data_access_id. Please provide a valid ID from a staging tool.",
							},
						],
					};
				}
			...
			async ({ data_access_id }) => {
				try {
					...
					if (schemaInfo.enhanced_schemas) {
						...
						Object.values(schemaInfo.enhanced_schemas).forEach(
							(schema: any) => {
								output += `### ${schema.table_name} Table:\n`;
								Object.entries(schema.columns).forEach(
									([colName, colInfo]: [string, any]) => {
					...
					if (schemaInfo.schema_guidance?.recommended_queries) {
						...
						schemaInfo.schema_guidance.recommended_queries.forEach(
							(rq: any) => {
