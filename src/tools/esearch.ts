	...
	register(): void {
		this.context.server.tool(
			...
			"Search Entrez databases with text queries to find matching UIDs. Core Entrez function that converts queries into UID lists for use with other E-utilities. Supports Boolean operators (AND, OR, NOT) and field-specific searches.\n\n🔍 **Common PubMed Fields**: [Title], [Author], [Journal], [MeSH], [Abstract], [Date], [DOI]\n🧬 **Sequence DB Fields**: [Organism], [Gene], [Protein], [Accession]\n🔬 **Search Examples**: \n• \"cancer[Title] AND therapy[Abstract]\" - Title contains cancer, abstract contains therapy\n• \"Smith J[Author]\" - Articles by author Smith J\n• \"2023[Date]\" - Articles from 2023\n• \"Nature[Journal]\" - Articles from Nature journal",
			{
				...
				retmode: z.enum(["xml", "json"]).optional().describe("Output format (auto-selected if not specified)"),
				intended_use: z.enum(["search", "staging", "analysis", "citation"]).optional().describe("Intended use case for optimal format selection"),
			},
			async ({ db, term, retstart, retmax, sort, field, usehistory, datetype, reldate, mindate, maxdate, retmode, intended_use }) => {
				try {
					...
					if (db && !this.isValidDatabase(db)) {
						throw new Error(`Invalid database name: ${db}. Use 'einfo' tool to see available databases.`);
					}
					...
					const selectedRetmode = retmode || this.context.getOptimalRetmode("esearch", dbName, intended_use);
					...
					const data = await this.parseResponse(response, "ESearch", selectedRetmode);

					// Get query suggestions
					const suggestions = this.context.suggestQueryImprovements(term, dbName);
