...
import { DataQualityMetrics } from './types.js';

export class DataInsertionEngine {
    async insertData(entities: { type: string; data: any }[], schemas: Record<string, any>, sql: any): Promise<DataQualityMetrics> {
        ...
        for (const [entityType, items] of Object.entries(typeGroups)) {
            ...
            for (const item of items) {
                try {
                    ...
                    quality.parsing_warnings.push(`Failed to insert ${entityType}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        ...
        if (quality.articles_processed > 0) {
            quality.mesh_success_rate = (quality.articles_with_mesh / quality.articles_processed) * 100;
        }

        return quality;
    }

    private async insertRelationships(typeGroups: Record<string, any[]>, schemas: Record<string, any>, sql: any, quality: DataQualityMetrics): Promise<void> {
                ...
                typeGroups.article.forEach(article => {
                    if (article.meshTerms && Array.isArray(article.meshTerms)) {
                        article.meshTerms.forEach((meshTerm: any) => {
                            if (meshTerm.uid) {
                                try {
                                    sql.exec('INSERT OR REPLACE INTO article_meshterm (article_uid, meshterm_uid) VALUES (?, ?)', 
                                        article.uid, meshTerm.uid);
                                } catch (error) {
                                    quality.parsing_warnings.push(`Failed to insert article-meshterm relationship: ${error instanceof Error ? error.message : String(error)}`);
                                }
                            }
                        });
                    } else {
                        quality.missing_relationships.push(`Article ${article.uid} has no MeSH term relationships`);
                    }
                });
            } catch (error) {
                quality.parsing_warnings.push(`Failed to create article-meshterm relationships: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
                ...
                typeGroups.article.forEach(article => {
                    if (article.authors && Array.isArray(article.authors)) {
                        article.authors.forEach((author: any) => {
                            if (author.uid) {
                                try {
                                    sql.exec('INSERT OR REPLACE INTO article_author (article_uid, author_uid) VALUES (?, ?)', 
                                        article.uid, author.uid);
                                } catch (error) {
                                    quality.parsing_warnings.push(`Failed to insert article-author relationship: ${error instanceof Error ? error.message : String(error)}`);
                                }
                            }
                        });
                    } else {
                        quality.missing_relationships.push(`Article ${article.uid} has no author relationships`);
                    }
                });
            } catch (error) {
                quality.parsing_warnings.push(`Failed to create article-author relationships: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
