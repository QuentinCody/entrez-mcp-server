...
import { ProcessingResult, DataQualityMetrics, ParsingDiagnostics, SchemaGuidance, RecommendedQuery, CommonJoin } from "./lib/types.js";
...
export class JsonToSqlDO extends DurableObject {
	...
	private async processAndStore(parseResult: any): Promise<ProcessingResult> {
		try {
			...
			const dataQuality = await this.dataEngine.insertData(entities, schemas, this.ctx.storage.sql);
			...
			return {
				success: true,
				message: "Data parsed and staged successfully into a relational database with enhanced schema guidance.",
			...
			return { 
                ...
                processing_details: {
                    ...
                    parsing_diagnostics: {
                        method_used: 'error',
                        terms_found: 0,
                        failed_extractions: [error instanceof Error ? error.message : String(error)],
    ...
    private generateSchemaGuidance(schemas: Record<string, any>): SchemaGuidance {
        ...
        if (schemas.article && schemas.meshterm) {
            commonJoins.push({
                ...
                join_condition: 'article.uid = article_meshterm.article_uid AND article_meshterm.meshterm_uid = meshterm.uid',
                example_sql: 'SELECT a.pmid, a.title, m.descriptorname FROM article a JOIN article_meshterm am ON a.uid = am.article_uid JOIN meshterm m ON am.meshterm_uid = m.uid ORDER BY a.year DESC'
            });
        }
        ...
        if (schemas.article && schemas.author) {
            commonJoins.push({
                ...
                join_condition: 'article.uid = article_author.article_uid AND article_author.author_uid = author.uid',
                example_sql: 'SELECT a.pmid, a.title, au.lastname, au.forename FROM article a JOIN article_author aa ON a.uid = aa.article_uid JOIN author au ON aa.author_uid = au.uid'
            });
        }
        ...
        const columnDescriptions = Object.values(enhancedSchemas).flatMap(schema => 
    ...
    private generateErrorSuggestions(errorMessage: string, sql: string): string[] {
        const suggestions: string[] = [];
        
        if (errorMessage.includes('no such table')) {
            suggestions.push("Check available tables with: SELECT name FROM sqlite_master WHERE type='table'");
            suggestions.push("Common tables: article, meshterm, author, article_meshterm, article_author");
        } else if (errorMessage.includes('no such column')) {
            suggestions.push("Check column names - remember they are lowercase (e.g., 'descriptorname', not 'DescriptorName')");
            suggestions.push("Use 'year' not 'publication_date', 'pmid' not 'pubmed_id'");
        ...
        if (sql.toLowerCase().includes('mesh')) {
            suggestions.push("For MeSH terms: JOIN article_meshterm am ON a.uid = am.article_uid JOIN meshterm m ON am.meshterm_uid = m.uid");
        }
        if (sql.toLowerCase().includes('author')) {
            suggestions.push("For authors: JOIN article_author aa ON a.uid = aa.article_uid JOIN author au ON aa.author_uid = au.uid");
        }
    ...
    private async getEnhancedSchema(): Promise<any> {
        const basicSchema = this.ctx.storage.sql.exec(`SELECT name, sql FROM sqlite_master WHERE type='table'`).toArray();
        const enhancedSchemas = this.schemaEngine.getEnhancedSchemas();
        
        return {
            basic_schema: basicSchema,
            enhanced_schemas: enhancedSchemas,
            schema_guidance: this.generateSchemaGuidance(this.schemaEngine.entitySchemas || {}),
            quick_start: {
                sample_queries: [
