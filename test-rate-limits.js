...
class NCBIRateLimitTester {
    ...
    async testRate(requestsPerSecond, duration, includeApiKey = false) {
        ...
        console.log(`   API Key: ${includeApiKey && NCBI_API_KEY ? '✅ Enabled' : '❌ Disabled'}`);
        ...
        console.log(`\n📊 Results:`);
        console.log(`   ✅ Successful: ${successful}/${totalRequests} (${testResult.successRate.toFixed(1)}%)`);
    ...
    async runTests() {
        ...
        if (!NCBI_API_KEY) {
            ...
            console.log("   To test authenticated limits, set NCBI_API_KEY environment variable.");
        } else {
    ...
    printSummary() {
        ...
        this.results.forEach((result, index) => {
            const apiKeyStatus = result.includeApiKey ? "✅ With API Key" : "❌ Without API Key";
            const status = result.successRate >= 90 ? "✅ PASS" : 
                          result.successRate >= 70 ? "⚠️ PARTIAL" : "❌ FAIL";
            
            console.log(`\nTest ${index + 1}: ${result.requestsPerSecond} req/sec (${apiKeyStatus})`);
            console.log(`  Status: ${status}`);
            console.log(`  Success Rate: ${result.successRate.toFixed(1)}%`);
            console.log(`  Rate Limited: ${result.rateLimited}/${result.totalRequests}`);
        ...
        if (noApiKeyResults.length > 0) {
            const bestNoApiKey = Math.max(...noApiKeyResults.filter(r => r.successRate >= 90).map(r => r.requestsPerSecond));
            if (bestNoApiKey > 0) {
                console.log(`  • Without API key: Keep requests ≤ ${bestNoApiKey}/second`);
            } else {
                console.log(`  • Without API key: All tested rates had issues - try lower rates`);
            }
        }
        
        if (withApiKeyResults.length > 0) {
            const bestWithApiKey = Math.max(...withApiKeyResults.filter(r => r.successRate >= 90).map(r => r.requestsPerSecond));
            if (bestWithApiKey > 0) {
                console.log(`  • With API key: Can safely use up to ${bestWithApiKey}/second`);
            } else {
